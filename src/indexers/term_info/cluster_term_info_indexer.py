...
class ClusterTermInfoQueryIndexer(BaseQueryIndexer):
    ...
    def get_vfb_json_query(self, ids: List[str]) -> str:
        ...
        # Updated query with hide_in_terminfo conditions
        ...
        OPTIONAL MATCH (o:Class)<-[r]-(primary) WHERE r.type='Related' AND ((not exists(r.hide_in_terminfo)) OR (not (r.hide_in_terminfo[0] = true)))
        ...
        OPTIONAL MATCH (o:Individual)<-[r]-(primary) WHERE r.type='Related' AND ((not exists(r.hide_in_terminfo)) OR (not (r.hide_in_terminfo[0] = true)))
